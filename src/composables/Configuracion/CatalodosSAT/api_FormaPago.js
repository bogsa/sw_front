...
export default function use_FormaPago() {
    const url = useUrl();
    const auth = useAuth();
    const httpRSC = use_httpRSC();

    const apiData_FormaPago = ref([]);
    const statusCode_FormaPago = ref(null);
    const get_FormaPago = async () => {
        apiData_FormaPago.value = [];
        try {
            const configuracion = {
                method: 'GET',
                url: url.apisw + 'api/C_FormaPago/Listar',
                headers: {
                    Authorization: 'Bearer: ' + auth.token,
                }
            }
            const respuesta = await axios(configuracion);
            apiData_FormaPago.value = respuesta.data;
            statusCode_FormaPago.value = respuesta.status;
        } catch (error) {
            let res_error = null
    ...
    const post_FormaPago = async (data) => {
        try {
            const configuracion = {
                method: 'POST',
                url: url.apisw + 'api/C_FormaPago/Crear',
                headers: {
    ...
    const put_FormaPago = async (data) => {
        try {
            const configuracion = {
                method: 'PUT',
                url: url.apisw + 'api/C_FormaPago/Actualizar',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer ' + auth.token
                },
