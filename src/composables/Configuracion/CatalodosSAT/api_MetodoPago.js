...
export default function use_MetodoPago() {
    const url = useUrl();
    const auth = useAuth();
    const httpRSC = use_httpRSC();

    const apiData_MetodoPago = ref([]);
    const statusCode_MetodoPago = ref(null);
    const get_MetodoPago = async () => {
        apiData_MetodoPago.value = [];
        try {
            const configuracion = {
                method: 'GET',
                url: url.apisw + 'api/C_MetodoPago/Listar',
                headers: {
                    Authorization: 'Bearer: ' + auth.token,
                }
            }
            const respuesta = await axios(configuracion);
            apiData_MetodoPago.value = respuesta.data;
            statusCode_MetodoPago.value = respuesta.status;
        } catch (error) {
            let res_error = null
    ...
    const post_MetodoPago = async (data) => {
        try {
            const configuracion = {
                method: 'POST',
                url: url.apisw + 'api/C_MetodoPago/Crear',
                headers: {
    ...
    const put_MetodoPago = async (data) => {
        try {
            const configuracion = {
                method: 'PUT',
                url: url.apisw + 'api/C_MetodoPago/Actualizar',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer ' + auth.token
                },
