...
export default function use_Moneda() {
    const url = useUrl();
    const auth = useAuth();
    const httpRSC = use_httpRSC();

    const apiData_Moneda = ref([]);
    const statusCode_Moneda = ref(null);
    const get_Moneda = async () => {
        apiData_Moneda.value = [];
        try {
            const configuracion = {
                method: 'GET',
                url: url.apisw + 'api/C_Moneda/Listar',
                headers: {
            ...
            apiData_Moneda.value = respuesta.data;
            statusCode_Moneda.value = respuesta.status;
        } catch (error) {
            let res_error = null
    ...
    const post_Moneda = async (data) => {
        try {
            const configuracion = {
                method: 'POST',
                url: url.apisw + 'api/C_Moneda/Crear',
                headers: {
    ...
    const put_Moneda = async (data) => {
        try {
            const configuracion = {
                method: 'PUT',
                url: url.apisw + 'api/C_Moneda/Actualizar',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer ' + auth.token,
                },
