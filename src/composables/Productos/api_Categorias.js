...
export default function use_Categoria() {
    const url = useUrl();
    const auth = useAuth();
    const httpRSC = use_httpRSC();

    const apiData_Categoria = ref([]);
    const statusCode_Categoria = ref(null);


    const get_Categoria = async (idCorporativo) => {
        apiData_Categoria.value = [];
        try {
            const configuracion = {
                method: "GET",
                url: url.apisw + 'api/C_Categoria/Listar/'+ idCorporativo,
                header: {
                    Ahutorization: 'Bearer ' + auth.token,
                }
            }
            const respuesta = await axios(configuracion);
            apiData_Categoria.value = respuesta.data;
    ...
    const post_Categoria = async (data) => {
        try {
            const configuracion = {
                method: 'POST',
                url: url.apisw + 'api/C_Categoria/Crear',
                headers: {
    ...
    const put_Categoria = async (data) => {
        debugger
        try {
            const configuracion = {
                method: 'PUT',
                url: url.apisw + 'api/C_Categoria/Actualizar',
                headers: {
                    'Content-Type': 'application/json',
