...
import use_FormaPago from '@/composables/Configuracion/CatalodosSAT/api_FormaPago.js'
...
const headers = [
    { key: 'idFormaPago', title: '#', sortable: true, },
...
const { apiData_FormaPago, statusCode_FormaPago, get_FormaPago, post_FormaPago, put_FormaPago } = use_FormaPago();
...
function limpiar() {
    formData.value.idFormaPago = null;
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idFormaPago = item.idFormaPago;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_FormaPago(formData.value)
            if (statusCode_FormaPago.value == 200) {
                await get_FormaPago();
                limpiar();
            }
        }
        else {
            await put_FormaPago(formData.value)
            if (statusCode_FormaPago.value == 200) {
                await get_FormaPago();
...
<template>
    ...
    <p class="text-h5 text-light">Forma de pago</p>
    <p class="text-caption text-light">Catálogo SAT de forma de pago.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_FormaPago.length }}
        </v-btn>
    ...
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_FormaPago"
