...
import use_Impuesto from '@/composables/Configuracion/CatalodosSAT/api_Impuesto.js'
...
const headers = [
    { key: 'idImpuesto', title: '#', sortable: true, },
...
const { apiData_Impuesto, statusCode_Impuesto, get_Impuesto, post_Impuesto, put_Impuesto} = use_Impuesto();
...
function limpiar() {
    formData.value.idImpuesto = null;
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idImpuesto = item.idImpuesto;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_Impuesto(formData.value)
            if (statusCode_Impuesto.value == 200) {
                await get_Impuesto();
                limpiar();
            }  
        }
        else {
            await put_Impuesto(formData.value)
            if(statusCode_Impuesto.value == 200)
            {
                await get_Impuesto();
...
<template>
    ...
    <p class="text-h5 text-light">Impuesto</p>
    <p class="text-caption text-light">Catálogo SAT de impuestos.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_Impuesto.length }}
        </v-btn>
    ...
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_Impuesto"
