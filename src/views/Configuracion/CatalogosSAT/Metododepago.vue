...
import use_MetodoPago from '@/composables/Configuracion/CatalodosSAT/api_MetodoPago.js'
...
const headers = [
    { key: 'idMetodoPago', title: '#', sortable: true, },
...
const { apiData_MetodoPago, statusCode_MetodoPago, get_MetodoPago, post_MetodoPago, put_MetodoPago} = use_MetodoPago();

onMounted(async () => {
    await get_MetodoPago();
...
function limpiar() {
    formData.value.idMetodoPago = null;
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idMetodoPago = item.idMetodoPago;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_MetodoPago(formData.value)
            if (statusCode_MetodoPago.value == 200) {
                await get_MetodoPago();
                limpiar();
            }  
        }
        else {
            await put_MetodoPago(formData.value)
            if(statusCode_MetodoPago.value == 200)
            {
                await get_MetodoPago();
...
<template>
    ...
    <p class="text-h5 text-light">Metodo de pago</p>
    <p class="text-caption text-light">Catálogo SAT de metodo de pago.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_MetodoPago.length }}
        </v-btn>

    </div>
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_MetodoPago"
