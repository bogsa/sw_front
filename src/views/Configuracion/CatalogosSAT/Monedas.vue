...
import use_Moneda from '@/composables/Configuracion/CatalodosSAT/api_Moneda.js'
...
const headers = [
    { key: 'idMoneda', title: '#', sortable: true, },
...
const { apiData_Moneda, statusCode_Moneda, get_Moneda, post_Moneda, put_Moneda} = use_Moneda();
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idMoneda = item.idMoneda;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_Moneda(formData.value)
            if (statusCode_Moneda.value == 200) {
                await get_Moneda();
                limpiar();
            }  
        }
        else {
            await put_Moneda(formData.value)
            if(statusCode_Moneda.value == 200)
            {
                await get_Moneda();
...
<template>
    ...
    <p class="text-h5 text-light">Moneda</p>
    <p class="text-caption text-light">Catálogo SAT de Monedas.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_Moneda.length }}
        </v-btn>

    </div>
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_Moneda"
