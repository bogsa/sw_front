...
import use_ObjetoImpuesto from '@/composables/Configuracion/CatalodosSAT/api_ObjetoImpuesto.js'
...
const headers = [
    { key: 'idObjetoImpuesto', title: '#', sortable: true, },
...
const { apiData_ObjetoImpuesto, statusCode_ObjetoImpuesto, get_ObjetoImpuesto, post_ObjetoImpuesto, put_ObjetoImpuesto} = use_ObjetoImpuesto();

onMounted(async () => {
    await get_ObjetoImpuesto();
...
function limpiar() {
    formData.value.idObjetoImpuesto = null;
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idObjetoImpuesto = item.idObjetoImpuesto;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_ObjetoImpuesto(formData.value)
            if (statusCode_ObjetoImpuesto.value == 200) {
                await get_ObjetoImpuesto();
                limpiar();
            }  
        }
        else {
            await put_ObjetoImpuesto(formData.value)
            if(statusCode_ObjetoImpuesto.value == 200)
            {
                await get_ObjetoImpuesto();
...
<template>
    ...
    <p class="text-h5 text-light">Objeto de impuesto</p>
    <p class="text-caption text-light">Catálogo SAT de Objeto de impuesto.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_ObjetoImpuesto.length }}
        </v-btn>

    </div>
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_ObjetoImpuesto"
