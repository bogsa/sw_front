...
import use_RegimenFiscal from '@/composables/Configuracion/CatalodosSAT/api_RegimenFiscal.js'
...
const headers = [
    { key: 'idRegimenFiscal', title: '#', sortable: true, },
...
const { apiData_RegimenFiscal, statusCode_RegimenFiscal, get_RegimenFiscal, post_RegimenFiscal, put_RegimenFiscal} = use_RegimenFiscal();

onMounted(async () => {
    await get_RegimenFiscal();
...
function limpiar() {
    formData.value.idRegimenFiscal = null;
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idRegimenFiscal = item.idRegimenFiscal;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_RegimenFiscal(formData.value)
            if (statusCode_RegimenFiscal.value == 200) {
                await get_RegimenFiscal();
                limpiar();
            }  
        }
        else {
            await put_RegimenFiscal(formData.value)
            if(statusCode_RegimenFiscal.value == 200)
            {
                await get_RegimenFiscal();
...
<template>
    ...
    <p class="text-h5 text-light">Régimen fiscal</p>
    <p class="text-caption text-light">Catálogo SAT de régimen fiscal</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_RegimenFiscal.length }}
        </v-btn>

    </div>
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_RegimenFiscal"
