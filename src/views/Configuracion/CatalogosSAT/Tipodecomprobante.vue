...
import use_TipoComprobante from '@/composables/Configuracion/CatalodosSAT/api_TipoComprobante.js'
...
const headers = [
    { key: 'idTipoComprobante', title: '#', sortable: true, },
...
const { apiData_TipoComprobante, statusCode_TipoComprobante, get_TipoComprobante, post_TipoComprobante, put_TipoComprobante} = use_TipoComprobante();

onMounted(async () => {
    await get_TipoComprobante();
...
function limpiar() {
    formData.value.idTipoComprobante = null;
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idTipoComprobante = item.idTipoComprobante;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_TipoComprobante(formData.value)
            if (statusCode_TipoComprobante.value == 200) {
                await get_TipoComprobante();
                limpiar();
            }  
        }
        else {
            await put_TipoComprobante(formData.value)
            if(statusCode_TipoComprobante.value == 200)
            {
                await get_TipoComprobante();
...
<template>
    ...
    <p class="text-h5 text-light">Tipo de comprobante</p>
    <p class="text-caption text-light">Catálogo SAT de Tipo de Comprobante.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_TipoComprobante.length }}
        </v-btn>

    </div>
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_TipoComprobante"
