...
import use_TipoFactor from '@/composables/Configuracion/CatalodosSAT/api_TipoFactor.js'
...
const headers = [
    { key: 'idTipoFactor', title: '#', sortable: true, },
...
const { apiData_TipoFactor, statusCode_TipoFactor, get_TipoFactor, post_TipoFactor, put_TipoFactor} = use_TipoFactor();

onMounted(async () => {
    await get_TipoFactor();
...
function limpiar() {
    formData.value.idTipoFactor = null;
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idTipoFactor = item.idTipoFactor;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_TipoFactor(formData.value)
            if (statusCode_TipoFactor.value == 200) {
                await get_TipoFactor();
                limpiar();
            }  
        }
        else {
            await put_TipoFactor(formData.value)
            if(statusCode_TipoFactor.value == 200)
            {
                await get_TipoFactor();
...
<template>
    ...
    <p class="text-h5 text-light">Tipo de  factor</p>
    <p class="text-caption text-light">Catálogo SAT de Tipo de factor.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_TipoFactor.length }}
        </v-btn>

    </div>
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_TipoFactor"
