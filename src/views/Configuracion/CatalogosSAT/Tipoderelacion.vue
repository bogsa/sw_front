...
import use_TipoRelacion from '@/composables/Configuracion/CatalodosSAT/api_TipoRelacion.js'
...
const headers = [
    { key: 'idTipoRelacion', title: '#', sortable: true, },
...
const { apiData_TipoRelacion, statusCode_TipoRelacion, get_TipoRelacion, post_TipoRelacion, put_TipoRelacion} = use_TipoRelacion();

onMounted(async () => {
    await get_TipoRelacion();
...
function limpiar() {
    formData.value.idTipoRelacion = null;
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idTipoRelacion = item.idTipoRelacion;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_TipoRelacion(formData.value)
            if (statusCode_TipoRelacion.value == 200) {
                await get_TipoRelacion();
                limpiar();
            }  
        }
        else {
            await put_TipoRelacion(formData.value)
            if(statusCode_TipoRelacion.value == 200)
            {
                await get_TipoRelacion();
...
<template>
    ...
    <p class="text-h5 text-light">Tipo de relación</p>
    <p class="text-caption text-light">Catálogo SAT de Tipo de relación.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_TipoRelacion.length }}
        </v-btn>

    </div>
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_TipoRelacion"
