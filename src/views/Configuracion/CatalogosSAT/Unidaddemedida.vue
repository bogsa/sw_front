...
import use_UnidadMedida from '@/composables/Configuracion/CatalodosSAT/api_UnidadMedida.js'
...
const headers = [
    { key: 'idUnidadMedida', title: '#', sortable: true, },
...
const { apiData_UnidadMedida, statusCode_UnidadMedida, get_UnidadMedida, post_UnidadMedida, put_UnidadMedida} = use_UnidadMedida();

onMounted(async () => {
    await get_UnidadMedida();
...
const v_descripcion = [
    ...
    value => {
        if (value?.length <= 100) return true
        return 'La descripcion debe de tener menos de 200 caracteres.'
    },
]
const v_nota = [
    value => {
    ...
    value => {
        ...
        return 'La nota debe de tener menos de 500 caracteres.'
    },
]
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_UnidadMedida(formData.value)
            if (statusCode_UnidadMedida.value == 200) {
                await get_UnidadMedida();
                limpiar();
            }  
        }
        else {
            await put_UnidadMedida(formData.value)
            if(statusCode_UnidadMedida.value == 200)
            {
                await get_UnidadMedida();
...
<template>
    ...
    <p class="text-h5 text-light">UnidadMedida</p>
    <p class="text-caption text-light">Catálogo SAT de UnidadMedidas.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0"  size="small" color="primary" variant="tonal" class="text-none mx-2"  >
            Registros: {{ apiData_UnidadMedida.length }}
        </v-btn>

    </div>
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_UnidadMedida"
        ...
        <v-form @submit.prevent="guardar">
            <v-card elevation="0" class=" text-secondary">
                <v-card-text>
                    ...
                    <v-text-field v-model="formData.nombre" class="mb-3" density="compact" color="accent"
                        variant="outlined" :counter="100" :rules="v_nombre">
                        <template v-slot:label>
                    ...
                    <v-text-field v-model="formData.nota" class="mb-3" density="compact" color="accent"
                        variant="outlined" :counter="500" :rules="v_nota">
                        <template v-slot:label>
                            <div class="font-weight-regular text-caption text-secondary">Nota:</div>
                        </template>
                    </v-text-field>
