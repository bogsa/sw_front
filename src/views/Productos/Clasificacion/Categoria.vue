...
import use_Categoria from '@/composables/Productos/api_Categorias'
...
const headers = [
    { key: 'idCategoria', title: '#', sortable: true, },
    { key: 'nombreDepartamento', title: 'Departamento', sortable: true, },
...
const { apiData_Categoria, statusCode_Categoria, get_Categoria, post_Categoria, put_Categoria } = use_Categoria();
const { apiData_Departamento, statusCode_Departamento, get_Departamento } = use_Departamento();

onMounted(async () => {
    await get_Categoria(id_corporativo.value);
...
function limpiar() {
    formData.value.idCategoria = null;
    formData.value.e_DepartamentoId = null;
...
function editar(item) {
    limpiar();
    editIndex.value = 1;
    formData.value.idCategoria = item.idCategoria;
    formData.value.e_DepartamentoId  = item.e_DepartamentoId;
...
const guardar = async (submitEventPromise: SubmitEventPromise) => {
    ...
    if (valid) {
        if (editIndex.value == -1) {
            await post_Categoria(formData.value)
            if (statusCode_Categoria.value == 200) {
                await get_Categoria(id_corporativo.value);
                limpiar();
            }
        }
        else {
            await put_Categoria(formData.value)
            if (statusCode_Categoria.value == 200) {
                await get_Categoria(id_corporativo.value);
...
<template>
    ...
    <p class="text-h5 text-light">Categorías</p>
    <p class="text-caption text-light">Categoria de productos.</p>
    ...
    <div class="text-subtitle-2 text-light my-2 text-medium-emphasis d-flex align-center justify-space-between">
        ...
        <v-btn elevation="0" size="small" color="primary" variant="tonal" class="text-none mx-2">
            Registros: {{ apiData_Categoria.length }}
        </v-btn>
    ...
    <v-card flat class="rounded-lg">
        <v-card-text>
            <v-data-table class="text-caption" :search="search" :headers="headers" :items="apiData_Categoria"
    ...
    <v-navigation-drawer app v-model="drawer" location="right" floating temporary permanent width="300">
        ...
        <v-form @submit.prevent="guardar">
            <v-card elevation="0" class=" text-secondary">
                <v-card-text>
                    <v-autocomplete placeholder="Selecciona" density="compact" label="Departamento" :items="apiData_Departamento"
                        v-model="formData.e_DepartamentoId" class="py-1" variant="outlined" color="accent" item-title="nombre"
                        item-value="idDepartamento" :rules="nameRulesSelect"  >
                    </v-autocomplete>
